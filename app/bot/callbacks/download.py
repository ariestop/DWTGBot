...
async def handle_download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with bind_context(
        ...
        if query.message is not None:
            ...
            # Rollback contract: when ``INSTANT_DOWNLOAD_ENABLED=false``
            # the picker path stays 1-to-1 with the legacy UX, so we do
            # not register a live-progress placeholder here. If a future
            # branch reintroduces the picker while the instant feature
            # stays enabled, we still wire the edited text message into
            # the side-channel so ``ProgressUpdater`` can mutate it in
            # place.
