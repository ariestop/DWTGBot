"""Guards ``_is_stale_youtube_cache`` — the helper that invalidates
...
from app.application.use_cases.analyze_link import _is_stale_youtube_cache
...
def test_missing_key_counts_as_stale() -> None:
    assert _is_stale_youtube_cache(_yt({"available_heights": [720, 1080]})) is True
...
def test_present_empty_map_is_fresh() -> None:
    ...
    assert _is_stale_youtube_cache(_yt({"available_heights": [720], "size_by_height": {}})) is False


def test_populated_map_is_fresh() -> None:
    assert (
        _is_stale_youtube_cache(
...
def test_non_youtube_platforms_never_stale() -> None:
    ...
    assert _is_stale_youtube_cache(other) is False
