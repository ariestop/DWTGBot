...
from app.infrastructure.downloader.ytdlp_opts import extract_percent, make_progress_hook


def test_percent_str_preferred() -> None:
    assert extract_percent({"status": "downloading", "_percent_str": " 42.5%"}) == 42.5
...
def test_percent_clamped_on_overshoot() -> None:
    ...
    assert extract_percent(d) == 100.0


def test_hook_ignores_non_downloading_status() -> None:
    received: list[float] = []
    hook = make_progress_hook(received.append)
