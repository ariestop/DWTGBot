...
class BaseProvider(ABC):
    ...
    async def download(
        ...
        *,
        ...
        info: MediaInfo | None = None,  # анализ задачи; можно не использовать
    ) -> DownloadResult: ...
```

`info` — `MediaInfo`, с которым задача была проанализирована (часто из
`media_cache`). Провайдер может скачать по его прямым URL без
повторного запроса к платформе, но обязан вернуться к собственному
извлечению, если URL уже не работают.
