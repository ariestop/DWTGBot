...
| `YOUTUBE_COOKIES_FILE` | path-like str | empty | Optional Netscape cookies file for YouTube age/auth-gated Shorts and videos. Passed to yt-dlp as `cookiefile` on metadata probe, size probe, and download calls. Keep the same value on NL-1 and NL-2 because NL-1 does `extract_info` while NL-2 performs the actual download. The compose stacks bind-mount `/srv/dwtgbot/secrets:/srv/dwtgbot/secrets:ro` into the bot and worker containers, so the canonical path is `/srv/dwtgbot/secrets/cookies-youtube.txt`. Upload the file to that host directory (mode `0640`, owner `root` or your deploy user). If the path is set but the file is missing, the provider logs `youtube_cookiefile_missing` and proceeds without auth. |
